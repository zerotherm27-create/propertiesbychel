...
export const LEAD_EMAIL_VOICE = `Section voice: A one-to-one email from the advisor to a specific prospective or existing client, not a marketing blast. Grounded strictly in the lead facts supplied below, nothing invented (no properties, prices, amenities, or promises not stated). Address the recipient by first name if a name is given, otherwise open plainly without inventing one. Short: 2 to 4 short paragraphs, the length of an email a busy person would actually write and a busy person would actually read, not an essay. The subject line should read like a person wrote it for this one recipient, never a marketing subject ("Don't miss out!", "Your dream home awaits"). Close with a single, natural next step that matches what this email is trying to accomplish, not a generic call to action.`;

export const EMAIL_TEMPLATE_VOICE = `Section voice: A reusable email template for an automated nurture sequence, sent to many different leads over time, not a single individual. Written for a stranger this practice has not yet spoken with, so it must not invent specifics about them: use the merge fields exactly as given (e.g. {firstName}, {intent}, {districts}, {budgetRange}, {timeframe}, {status}, {listingTitle}) wherever a personal detail would naturally go, rather than writing something generic in its place. Short: 2 to 4 short paragraphs. The subject line and heading should read like they were written by a person for one recipient, never a mass-marketing subject ("Don't miss out!", "Limited time"). Close with a single, natural next step, reflected in a short button label (a few words, e.g. "See the listing" or "Book a call"), not a generic call to action.`;

export const FLOW_VOICE = `Section voice: designing the shape of a multi-touch automated email nurture flow for a boutique real-estate advisory (not a high-volume marketing operation) — sparing and considered, not a hard drip campaign. Each email in it uses ${"{firstName}"}, ${"{intent}"}, ${"{districts}"}, ${"{budgetRange}"}, ${"{timeframe}"}, ${"{status}"}, ${"{listingTitle}"} as merge fields where a personal detail belongs, same as any other template. A typical flow is 3 to 5 emails: the first with no wait (delay_days 0, sent the same day the lead enrolls — the flow only runs once a day, so never write copy that promises an instant or immediate reply), then increasing gaps between the rest (a few days, then a week or more) so it reads as patient and personal rather than an automated blast. delay_days is always counted from enrollment, not from the previous email, so values must be ascending across steps, not deltas between them.`;
